...
ax.set_xlabel(r"SOC strength $\lambda$", fontsize=13)
ax.set_ylabel(r"$|E_{[110]} - E_{[001]}|$", fontsize=13)
...
ax = axes[1]
ax.plot(df["lam"], df["moment_110"],
        color="tomato", linewidth=1.5, label=r"$\hat{n} = [110]$", zorder=1)
ax.scatter(df["lam"], df["moment_110"],
           color="tomato", edgecolors="white",
           s=60, linewidths=1.2, zorder=2)
ax.plot(df["lam"], df["moment_001"],
        color="steelblue", linewidth=1.5, label=r"$\hat{n} = [001]$", zorder=1)
ax.scatter(df["lam"], df["moment_001"],
           color="steelblue", edgecolors="white",
           s=60, linewidths=1.2, zorder=2)
ax.set_xlabel(r"SOC strength $\lambda$", fontsize=13)
ax.set_ylabel(r"Moment $\langle n_\uparrow - n_\downarrow \rangle$", fontsize=13)
